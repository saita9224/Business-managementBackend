...
class POSMutation:
    ...
    async def open_pos_session(self, info: Info, input: OpenSessionInput) -> POSSessionType:
    ...
    async def close_pos_session(self, info: Info, input: CloseSessionInput) -> POSSessionType:
        try:
    ...
    async def create_order(self, info: Info, receipt_id: strawberry.ID) -> OrderType:
        user = info.context.user
        try:
            # ── BUG FIX: was doing sync DB call Receipt.objects.get on async thread
    ...
    @strawberry.mutation
    @permission_required("pos.create_order")
    async def add_order_item(self, info: Info, input: AddOrderItemInput) -> OrderItemType:
        user = info.context.user
        try:
            # ── BUG FIX: was doing sync DB calls on async thread
    ...
    async def finalize_receipt(self, info: Info, receipt_id: strawberry.ID) -> ReceiptType:
        user = info.context.user
        try:
            receipt = await sync_to_async(Receipt.objects.get)(pk=int(receipt_id))
    ...
    async def accept_payment(self, info: Info, input: AcceptPaymentInput) -> PaymentType:
        user = info.context.user
        try:
    ...
    async def create_credit(self, info: Info, input: CreateCreditInput) -> CreditAccountType:
        user = info.context.user
        try:
    ...
    async def refund_receipt(self, info: Info, input: RefundReceiptInput) -> ReceiptType:
        user = info.context.user
        try:
